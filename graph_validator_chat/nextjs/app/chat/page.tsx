...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      setStatus('Checking pipeline...');
      const bootstrap = await bootstrapPipelineIfNeeded();
      if (!bootstrap.initialized && bootstrap.error) {
        setPipelineMissing(true);
        setStatus('Pipeline not initialized. Please upload a file to start.');
  ...
  const checkStatus = async () => {
    try {
      ...
      if (data.initialized) {
        ...
        setInputDisabled(true);
        setStatus(pipelineMissing ? 'Pipeline not initialized. Please upload a file to start.' : 'Initializing Backend...');
      }
    } catch (error) {
      console.error('Error checking status:', error);
      setIsBackendReady(false);
      setInputDisabled(true);
      setStatus(pipelineMissing ? 'Pipeline not initialized. Please upload a file to start.' : 'Waiting for server...');
    }
  };
